...
def test_refresh_and_clear_loop_env(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    import os

    from tokensaver_egress.business_loop import (
        clear_current_loop_file,
    ...
    write_current_loop_file(loop_id="loop_hot", loop_kind="goal_based", iteration=1)
    data = refresh_loop_env_from_file()
    assert data["TOKENSAVER_LOOP_ID"] == "loop_hot"
    assert os.environ["TOKENSAVER_LOOP_ID"] == "loop_hot"

    clear_current_loop_file()
    stub = tmp_path / "current-loop.env"
    assert stub.is_file()
    assert "TOKENSAVER_LOOP_ID" not in stub.read_text(encoding="utf-8")
    assert refresh_loop_env_from_file() == {}
