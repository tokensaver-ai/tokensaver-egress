...
def test_ensure_ingest_url_local(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("TOKENSAVER_INGEST_URL", raising=False)
    monkeypatch.setattr("builtins.input", lambda *_a, **_k: "2")
    assert wizard.ensure_ingest_url() == wizard.LOCAL_INGEST_URL
    assert wizard.os.environ["TOKENSAVER_INGEST_URL"] == wizard.LOCAL_INGEST_URL
