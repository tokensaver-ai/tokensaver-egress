...
7. **Port** (default `8888`)
8. **Claude Code skills** (CCR / onboarding) if missing — no `tokensaver-cli` required
...
| `loop status` iters=0 while Flux shows Anthropic traffic | Captures missing `attrs.loop_id` (fixed in **0.1.25+**). Upgrade egress, restart `serve --loop --goal "…"`, send a new Claude turn. |
