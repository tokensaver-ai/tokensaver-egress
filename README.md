...
Starts the proxy in the background if needed, then launches Claude Code. When Claude exits, the auto-proxy stops (unless you used `--keep-proxy`).
...
| Shell still pointing at `:8888` | `eval "$(tokensaver-egress unproxy --sh)"` |
