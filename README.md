...
Advanced (two terminals): keep `tokensaver-egress serve` running, then
`source ~/.tokensaver-egress/client-env.sh && claude` in another shell.
...
| `tokensaver-egress serve` | Start proxy only on `0.0.0.0:8888` |
