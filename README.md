...
### Account options in `setup` (API key)

The wizard asks how to obtain a `TOKENSAVER_API_KEY` (`ts_…`):

| # | Option | Who | What happens |
|---|--------|-----|----------------|
| **1** | **Create a free account** | New users | `POST /api/v1/auths/signup` with `create_key=true` → key returned once |
| **2** | **Log in** (default) | Existing email/password account | `POST /api/v1/auths/signin` → JWT → `POST /api/v1/api-keys` (label **Egress CLI**) → key returned once |
| **3** | **Paste** an existing key | CI / SSO / you already have a key | You paste `ts_…` (Settings → TokenSaver API keys) |
| **4** | **Open** the console | Browser path | Opens https://platform.tokensaver.fr |

Choose **SaaS** (`https://api.tokensaver.fr`) or **Local** (`http://localhost:8000`) before signup/login.

**Log in (option 2) — typical flow:**

```text
tokensaver-egress setup
→ Keep this key? n          # if replacing an old key
→ 2) Log in …
→ 1) TokenSaver SaaS
→ email + password
→ API key saved in ~/.tokensaver-egress/env
```

Notes:

- Login **creates a new** API key each time (counts toward plan key quota). Prefer **paste** if you already have a key you want to reuse.
- Email must be **verified** to create keys via login (same rule as the console).
- **SSO-only** orgs (local password disabled): use **paste** after IdP login in the console — not email/password login.
- Set ``TOKENSAVER_ENTERPRISE=1`` and/or ``TOKENSAVER_ORG_HINT=<slug>`` to hide **create account** (login + paste remain). See platform docs **IDENTITY-MANAGEMENT-ET-ENTERPRISE-V2**.

The wizard then continues with:

1. **Ingest target** — SaaS, **Local** (`http://localhost:8000/…/ingest`), or custom URL
2. **Loop precheck** default (`TOKENSAVER_LOOP_PRECHECK`) for Boucles
3. **MITM** on/off + create the local CA if needed
4. **Trust instructions** (and optional macOS keychain trust)
5. **Body capture** on/off
6. **Port** (default `8888`)
7. **Claude Code skills** (CCR / onboarding / Boucles) if missing — no `tokensaver-cli` required
