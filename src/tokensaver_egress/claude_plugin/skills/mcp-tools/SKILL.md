...
**Automatic:** for any coding task with a done condition, follow skills
**`tokensaver-agentic-loops`** (start) and **`tokensaver-close-loop`** (end).
Use MCP only — do **not** ask the user to run `tokensaver loop start|end`.

| Tool | Use when |
|------|----------|
| `tokensaver_loop_start` | Before first implementing tool call (auto) |
| `tokensaver_loop_iteration` | Milestone |
| `tokensaver_loop_precheck` | Before expensive turn (policy) |
| `tokensaver_loop_end` | **Before** saying done / abort (mandatory) |
| `tokensaver_loop_get` | Inspect loop |

Console: **Espace développeur** / graphe **Boucles**. Stop hook may also end a leftover Boucle.
