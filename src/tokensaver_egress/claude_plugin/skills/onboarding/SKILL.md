...
| **Business loops** | Auto via skills `tokensaver-agentic-loops` + `tokensaver-close-loop` (MCP) whenever you implement behind egress or have a done condition — stamps `current-loop.env` so Flux IA Boucles get the traffic. User never runs `tokensaver loop start/end`. |
