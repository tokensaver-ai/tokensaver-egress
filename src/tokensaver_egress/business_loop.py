...
_LOOP_ENV_KEYS = (ENV_LOOP_ID, ENV_LOOP_KIND, ENV_LOOP_ITERATION, ENV_LOOP_PRECHECK)
...
def write_current_loop_file(
    *,
    ...
    """Persist active loop for egress / CLI (Claude MCP or ``serve --loop``)."""
    from tokensaver_egress.wizard import CONFIG_DIR

    path = CONFIG_DIR / "current-loop.env"
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Written by TokenSaver (MCP tokensaver_loop_start or egress --loop)",
...
def clear_current_loop_file() -> None:
    """Mark the stamp file cleared and drop LOOP_* from the process env.

    Writes a stub file (no ``TOKENSAVER_LOOP_ID``) so a running ``serve``
    hot-reload unbinds; deleting alone would leave the process env stamping.
    """
    from tokensaver_egress.wizard import CONFIG_DIR
    ...
    path = CONFIG_DIR / "current-loop.env"
    try:
        path.write_text("# cleared by tokensaver_loop_end\n", encoding="utf-8")
        path.chmod(0o600)
    except OSError:
        pass
    for key in _LOOP_ENV_KEYS:
...
def refresh_loop_env_from_file() -> dict[str, str]:
    """Hot-reload LOOP_* from ``current-loop.env`` (Claude MCP while serve runs).

    - File with ``TOKENSAVER_LOOP_ID`` → overwrite process env (MCP start).
    - Cleared stub (no id) → drop LOOP_* (MCP end).
    - Missing file → leave process env alone (``serve --loop`` / shell export).
    """
    from tokensaver_egress.wizard import CONFIG_DIR
    ...
    if not path.is_file():
        return {}
    data = load_current_loop_file(only_if_unset=False)
    if not (data.get(ENV_LOOP_ID) or "").strip():
        for key in _LOOP_ENV_KEYS:
            os.environ.pop(key, None)
        return {}
    return data
...
def start_business_loop(
    *,
    ...
    goal_text = (goal or "").strip() or None
    # Org policies often set require_goal_expression_for=["goal_based"] — precheck
    # then returns GOAL_REQUIRED / 403 and Claude looks "broken". Fail fast here.
    if kind == "goal_based" and not goal_text:
        raise RuntimeError(
            "goal_based loops require --goal \"…\" (org policy GOAL_REQUIRED).\n"
            "  Example:  tokensaver-egress serve --loop --goal \"tests green\"\n"
            "  Or:       tokensaver-egress claude --loop --goal \"tests green\"\n"
            "  Or use:   --loop-kind turn_based   (no goal expression required)"
        )

    lid = f"loop_{uuid.uuid4().hex[:24]}"
    goal_obj: dict[str, Any] | None = None
    if goal_text or max_turns:
        goal_obj = {}
        if goal_text:
            goal_obj["expression"] = goal_text
