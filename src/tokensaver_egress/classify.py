...
def _extract_llm(body: dict[str, Any], provider: str | None) -> EgressClassification:
    ...
    # Anthropic prompt caching: input_tokens is uncached-only.
    if isinstance(tokens_input, int) or usage.get("cache_read_input_tokens") or usage.get(
        "cache_creation_input_tokens"
    ):
        try:
            base = int(tokens_input or 0)
        ...
        try:
            ...
            cache_c = 0
        if cache_r or cache_c or isinstance(tokens_input, int):
            tokens_input = base + cache_r + cache_c
