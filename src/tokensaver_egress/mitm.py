...
def _loop_from_headers(headers: dict[str, str] | None) -> str | None:
    ...
    Prefer ``X-Tokensaver-Loop-Id`` when present; else hot-reload
    ``~/.tokensaver-egress/current-loop.env`` (Claude MCP start/end) then
    ``TOKENSAVER_LOOP_ID``. An empty/None headers dict must not skip the
    file/env fallback — that left Boucles at iters=0 while Flux still showed
    captures.
    """
    if headers:
        for k, v in headers.items():
