...
def _scan_usage(meta: dict[str, Any], data: bytes) -> None:
    ...
    Anthropic emits ``input_tokens`` in ``message_start`` (head) and the final
    ``output_tokens`` in ``message_delta`` (tail); OpenAI emits ``usage`` in the last
    chunk before ``[DONE]``. We keep the model (first seen) and the max token counts.
    """
    ...
    try:
        ...
        if client_abort:
            ...
            logger.info(
                ...
                "trust ~/.tokensaver-egress/ca/ca.crt "
                "(Node/Claude Code: export NODE_EXTRA_CA_CERTS=$HOME/.tokensaver-egress/ca/ca.crt)",
                host,
                type(exc).__name__,
            )
        else:
...
async def _emit_loop_blocked(
    ...
    payload = {
        "error": {
            ...
            "message": f"Blocked by TokenSaver loop precheck: {getattr(verdict, 'effect', 'deny')}",
            "precheck": getattr(verdict, "body", {}) or {},
        }
    }
    body = json.dumps(payload).encode("utf-8")
    reason = "Forbidden" if code != 429 else "Too Many Requests"
    head = (
...
async def _relay_one_exchange(
    ...
    try:
        ...
        if live_stream_translate:
            ...
            stream_meta, full_body = await _buffer_response_body(up_reader, resp_headers)
            ...
            await client_writer.drain()
            stream_meta, head_sample = await _stream_response_body(up_reader, client_writer, resp_headers)
