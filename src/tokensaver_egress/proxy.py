...
from tokensaver_egress.mitm import default_execution_trace_id, maybe_handle_connect
...
async def serve(
    ...
    if loop_id:
        ...
        logger.info(
            "BusinessLoop binding: none (export TOKENSAVER_LOOP_ID before serve "
            "to attach captures to a Boucle)"
        )
    async with server:
