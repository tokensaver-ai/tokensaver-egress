...
Everyday (after setup once):
  tokensaver-egress serve                  # terminal A — logs
  tokensaver-egress claude --no-start      # terminal B — Claude
  tokensaver-egress serve --loop           # same + create BusinessLoop
  tokensaver-egress claude --loop          # one-shot: loop + proxy + Claude
...
Options (serve):
  ...
  --loop               Start a BusinessLoop and bind TOKENSAVER_LOOP_ID
  --loop-kind KIND     goal_based (default) | turn_based | …
  --max-turns N        Loop budget (default: 5)
  --goal TEXT          Goal expression for the loop
...
Options (claude / run):
  ...
  --loop               (claude) Start a BusinessLoop and bind TOKENSAVER_LOOP_ID
  --loop-kind KIND     goal_based (default) | turn_based | …
  --max-turns N        Loop budget (default: 5)
  --goal TEXT          Goal expression for the loop
...
Env:
  ...
  TOKENSAVER_LOOP_ID / TOKENSAVER_LOOP_PRECHECK  Attach captures to a Boucle
...
def _cmd_serve(args: argparse.Namespace) -> int:
    return run_serve(
        ...
        with_loop=bool(getattr(args, "loop", False)),
        loop_kind=str(getattr(args, "loop_kind", None) or "goal_based"),
        loop_max_turns=getattr(args, "max_turns", 5),
        loop_goal=getattr(args, "goal", None),
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    serve_p.add_argument(
        "--loop",
        action="store_true",
        help="Start a BusinessLoop and bind TOKENSAVER_LOOP_ID on this proxy",
    )
    serve_p.add_argument("--loop-kind", default="goal_based", help="Loop kind (default: goal_based)")
    serve_p.add_argument(
        "--max-turns",
        type=int,
        default=5,
        help="Max turns for --loop (default: 5)",
    )
    serve_p.add_argument("--goal", default=None, help="Goal expression for --loop")
    ...
    for name, help_txt in (
        ...
        if name == "claude":
            p.add_argument(
                "--loop",
                action="store_true",
                help="Start a BusinessLoop and bind TOKENSAVER_LOOP_ID on the proxy",
            )
            p.add_argument(
                "--loop-kind",
                default="goal_based",
                help="Loop kind (default: goal_based)",
            )
            p.add_argument(
                "--max-turns",
                type=int,
                default=5,
                help="Max turns / iterations for --loop (default: 5)",
            )
            p.add_argument(
                "--goal",
                default=None,
                help="Goal expression for --loop",
            )
        p.add_argument(
            "argv",
