...
def _obtain_api_key_interactive() -> str:
    """First-time path: create account (self-serve), paste key, or open the console."""
    _out()
    if is_enterprise_egress_mode():
        hint = (os.environ.get("TOKENSAVER_ORG_HINT") or "").strip()
        _info("Enterprise mode — create-account is disabled for this tenant.")
        if hint:
            _info(f"  TOKENSAVER_ORG_HINT={hint}")
        _info("Paste a ts_… key from your console (SSO user → Settings → API keys).")
        choice = _prompt_choice(
            "  How do you want to continue?",
            [
                ("1", "Paste an existing TOKENSAVER_API_KEY"),
                ("2", f"Open {PLATFORM_URL} in the browser"),
            ],
            default="1",
        )
        if choice == "2":
    ...
    choice = _prompt_choice(
        ...
        [
            ("1", "Create a free TokenSaver account  (recommended)"),
    ...
    if choice == "1":
        ...
        if key:
            return key
        _warn("Account setup did not finish — you can paste a key instead.")
    elif choice == "3":
        _info("Create a key at:  " + PLATFORM_URL)
        try:
            import webbrowser
...
def _signup_for_api_key() -> str:
    ...
    target = _prompt_choice(
        "  Where should we create the account?",
        [
    ...
    api_origin = LOCAL_API_ORIGIN if target == "2" else DEFAULT_API_ORIGIN
    # Align ingest URL with the account target (user can still change it next step).
    if target == "1":
    ...
    try:
        ...
        if "already exists" in str(exc).lower():
            _info("If you already have an account, paste an existing key (option 2).")
        return ""
...
def ensure_loop_precheck() -> bool:
    """Persist soft precheck for BusinessLoops (ACP-9) when TOKENSAVER_LOOP_ID is set."""
    _out()
    _out("  Business loops (Boucles): soft precheck when TOKENSAVER_LOOP_ID is set.")
    _out("  Use later:  tokensaver-egress claude --loop")
...
def print_client_howto(port: int, ca: MitmCA, *, mitm: bool) -> None:
    ...
    _out()
    _out("  Easiest next step (one command, no second terminal):")
    _out("  ───────────────────────────────────────────────────")
    _out(f"    tokensaver-egress claude          # proxy auto on :{port}")
    _out("    tokensaver-egress claude --loop    # + create BusinessLoop (Boucles)")
    _out("    # or:  tokensaver-egress run -- curl -I https://example.com")
    _out()
    _out("  Advanced: keep `serve` running, then in another terminal:")
    _out("  ─────────────────────────────────────────────────────────")
    _out(f"    source {path}")
    if mitm:
        _out(f'    # NODE_EXTRA_CA_CERTS is set by the script → {ca.ca_cert_path}')
    _out("    claude")
    _out()
    _out("  After sourcing client-env manually, clear with:")
...
def run_guided_setup(*, start_after: bool = True) -> int:
    ...
    if not start_after:
        _ok("Setup complete. Next:  tokensaver-egress claude")
        _info("With Boucles:      tokensaver-egress claude --loop")
        return 0

    if not _prompt_yes_no("  Launch Claude Code through the proxy now?", default=True):
        _info("OK — when ready:  tokensaver-egress claude")
        _info("         Boucles:  tokensaver-egress claude --loop")
        return 0

    use_loop = _prompt_yes_no("  Start a BusinessLoop for this session (--loop)?", default=False)
