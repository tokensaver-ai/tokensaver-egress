...
def print_client_howto(port: int, ca: MitmCA, *, mitm: bool) -> None:
    ...
    _out()
    _out("    # Or two shells (logs visible)")
    _out(f"    tokensaver-egress serve            # terminal 1 — :{port}")
    _out("    tokensaver-egress claude --no-start # terminal 2")
    _out()
    _out("    # Or any command")
    _out("    tokensaver-egress run -- curl -I https://example.com")
    _out()
