...
def ensure_api_key() -> str:
    current = (os.environ.get("TOKENSAVER_API_KEY") or "").strip()
    if current:
    ...
    if not current:
        ...
        while True:
            entered = _prompt("  Paste your TOKENSAVER_API_KEY", secret=True)
            if entered:
...
def persist_current_config(port: int) -> Path:
    values = {
        "TOKENSAVER_API_KEY": (os.environ.get("TOKENSAVER_API_KEY") or "").strip(),
