...
- Sync from monorepo packages/egress
...
- Docs / wizard: **Quick start** with one-shell (``claude``) and two-shell
  (``serve`` + ``claude --no-start``); TOKENSAVER ASCII banner in README;
  ``--loop`` no longer the default path. (0.1.42–0.1.43: OSS auto-bumps)

## [0.1.41] - 2026-09-26

- Guided setup: **Log in to an existing account** (email + password) →
  ``POST /auths/signin`` then ``POST /api-keys`` to mint an egress ``ts_…`` key
  (no console paste required for password accounts).
- Menu: (1) create account · (2) **log in** (default) · (3) paste key · (4) open browser.
- Enterprise mode: login + paste (public signup still hidden).
- Docs: README **Quick start** leads with two terminals (`serve` + `claude --no-start`);
  ``--loop`` no longer pushed as the default path.
- Docs: README account section, runbook, identity V2, PyPI release notes.

## [0.1.40] - 2026-09-26

- Sync / patch release from monorepo (pre-login wizard).
...
- Guided setup: **create a free TokenSaver account** from the CLI (name, email,
  password) via ``POST /auths/signup`` with ``create_key=true`` — no need to open
  the console first. Also offers paste-key and open-browser fallbacks.
- Docs: README signup steps + troubleshooting ``verification_failed`` / Developer
  workspace; monorepo runbook §2.1 / §2.2 aligned.

## [0.1.35] - 2026-09-20

- « N tours » labs: mandatory ``tokensaver_loop_iteration`` after **each** tour
  (continue/stop). Ban batching all writes then a single end (was showing 1/N).

## [0.1.34] - 2026-09-20

- Lab ``/goal`` labels (``three_steps == 1``, file tours): start **without**
  ``metric_key``; always end ``work_complete``; never invent metrics or retry
  ``goal_met`` after a successful close. Hook text updated to match.

## [0.1.33] - 2026-09-20

- Remove the Stop hook that injected ``additionalContext``. Claude Code treats
  that as « continue », so ``/goal`` reopened a second Boucle and rewrote the lab.
  Start/end stay in the skill only. Skill: one ``/goal`` = one Boucle; retry
  start once on timeout; never backfill with a second loop.

## [0.1.32] - 2026-09-20

- Skills/hooks: default ``tokensaver_loop_end`` to ``work_complete`` for Claude
  ``/goal`` file labs; ban ``goal_met`` without ``metrics`` (prevents
  ``METRIC_MISSING`` / ``verification_failed`` while Claude shows Goal achieved).

## [0.1.31] - 2026-09-20

- Silent Boucles for Claude users: skill rewrite (no MCP in user prompts) +
  hooks ``UserPromptSubmit`` / ``UserPromptExpansion`` (``/goal``) inject
  start/iterate/end reminders; ``Stop`` reminds to close an open stamp.
- Hook scripts ``remind-boucle.sh`` / ``remind-close-boucle.sh``; hooks.json
  uses the official nested ``hooks`` schema.

## [0.1.30] - 2026-09-20

- Claude skills: egress-aware auto BusinessLoops — start when implementing behind
  egress (or on ``MISSING_LOOP_ID``), reuse ``claude --loop`` stamp, iterate +
  precheck, end clears ``current-loop.env``. Updated ``agentic-loops``,
  ``close-loop``, ``mcp-tools``, ``onboarding``, and ``/loop`` command.

## [0.1.29] - 2026-09-19

- **Fix:** ``setup`` / ``serve`` now prefer ``TOKENSAVER_API_KEY`` +
  ``TOKENSAVER_INGEST_URL`` from ``~/.tokensaver-egress/env`` over a leftover
  shell export (monorepo ``.env`` / Cursor), so changing the key in setup
